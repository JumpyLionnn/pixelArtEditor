...
class ColorPalette extends BasicCanvas {
    ...
    public drawSelector(position: Vector2){
        this.ctx.beginPath();
        const color = Color.hsvToRgb(this.hue, (position.x / this._width) * 100, (1 - position.y / this._height) * 100).toString();
    ...
    public drawGradient(){
        ...
        const color = Color.hsvToRgb(this.hue, 100, 100).toString();
        this._style.innerHTML = `#color-picker-hue::-webkit-slider-thumb {background-color: ${color};}`;
        this.ctx.fillStyle = color;
